...
public class GestorPesistenciaPassword implements GestorPersistenciaPasswordIntefaz {
	...
	@Override
	public int permisoLectura(Long id) {
		Connection conn = null;
		ResultSet rs = null;
		int resultado = 0;
		try {
			conn = getConnection();
			Statement statement = conn.createStatement();
			rs = statement.executeQuery("SELECT lectura FROM permisosPassword WHERE idUser = " + id + "");
	...
	@Override
	public int permisoEscritura(Long id) {
		Connection conn = null;
		ResultSet rs = null;
		int resultado = 0;
		try {
			conn = getConnection();
			Statement statement = conn.createStatement();
			rs = statement.executeQuery("SELECT escritura FROM permisosPassword WHERE idUser = " + id + "");
	...
	@Override
	public Usuario comprobarUsuario(Long uid) {
		Connection conn = null;
		ResultSet rs = null;
		Usuario id = null;
		try {
			conn = getConnection();
			Statement statement = conn.createStatement();
			rs = statement.executeQuery("SELECT idUser FROM permisosPassword WHERE idUser = " + uid + "");

			if (rs.next()) {
				id = new Usuario(rs.getLong("idUser"));
			}
			return id;
